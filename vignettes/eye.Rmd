...
- Visual acuity conversion between Snellen, logMAR and ETDRS [`va()`](#va)
- Counting patients and eyes [`eyes()`](#eyes)
- Reshaping data from wide to long [`myop()`](#myop)
- Summarising data with common statistics (mean, sd, n, range)  [`reveal()`](#reveal)
- [`blink()`](#blink) wraps the above four functions into one call.
- Calculating age of patients [`age()`](#age)

*eye* includes [`amd`](#amd-data), a real life data set of people who received intravitreal injections due to age-related macular degeneration in **Moorfields Eye Hospital**. (Fasler et al. 2019)

*eye* includes a [conversion chart](#va-conversion-chart).

This vignette provides more detailed information about the functions in this package. Specific requirements will be explained, and examples provided. 

## AMD data
`amd` is an anonymised real life data set from a large cohort of patients with treatment-naive neovascular age-related macular degeneration (AMD) who received intravitreal anti-VEGF therapy in Moorfields Eye Hospital, London, UK. [The original data can be found here.](https://datadryad.org/stash/dataset/doi:10.5061/dryad.97r9289)</br>
...
`va()` Cleans and converts visual acuity notations (classes) between Snellen (decimal, meter and feet), ETDRS, and logMAR. Each class can be converted from one to another, and va() converts to logMAR by default. `va()` will detect the class automatically based on specific rules detailed below.

It takes an (atomic) vector with visual acuity entries as required argument. 
...
### Cleaning
- `NA` are assigned to missing entries or strings representing such entries (".", "", "{any number of spaces}", "N/A", "NA", "NULL")
- "plus" and "minus" from Snellen entries are removed
- Notation for qualitative entries is simplified (NPL becomes NLP, PL becomes LP).
...
### Detection
 ...
 - if x numeric and all x in intersection(va_chart$logMAR, chart$snellen_dec):
...
`eyes` offers a very simple API for counting patients and eyes. 
...
### Guessing
Column names can be in any case. 
...
### Counting 
For counting eyes, eyes need to be coded in commonly used ways.

- `eyes` recognizes integer coding 0:1 and 1:2, with right being
   the lower number. 
- Or, arguably more appropriate in R, character coding for a categorical variable: `eyes` recognizes right eyes: c("r", "re", "od", "right") and
...
#### Use in rmarkdown
`eyestr` was designed with the use in rmarkdown in mind, most explicitely for the use inline:
...
Out of convenience, data is often entered in a "wide" format: In particular, in eye research, there will be often two columns for the same variable, one column for each eye. 

However, "eye" is a variable (a dimension of your observation), and it should ideally be stored in one column. (It may be necessary to pivot right and left eye values into different columns for specific questions, but in my experience it is better to do this type of data transformation ad hoc only, meaning for the purpose).

[Learn about tidy data.](https://tidyr.tidyverse.org/articles/tidy-data.html)

`myop()` will make your data "tidier". It will remove duplicate rows, and pivot the eye variable to one column and generate a single column for each variable, thus tidying the data for specific types of analysis. For example, eight columns that store data of four variables for right and left eyes will be pivoted to 5 columns (one eye column and four further variable columns)). See also **Examples**.

As with `eyes()`, `myop()` requires a specific data format. 
...
myop will only work properly if you adhere to the following:
...
1. Myopization: The actual work is done with `myopizer()` and `myop_pivot()` and itself consiste of three steps. 
...
1. Rename data names with `myop_rename()` and `sort_substr()`
1. Getting the new names based on the indices of the colums
...
As you can see, a lot of those steps rely hugely on reasonnable naming of your columns. 
...
If you struggle to rename your columns, please have a look at those two interesting Stackoverflow threads which suggest many different ways to rename one or more columns: 
...
It is not really intended to replace other awesome data exploration packages / functions such as `skimr::skim`, and it will likely remain focussed on summarising numerical data only. 
...
This vignette is not meant to dive into detail how to use `ggplot2`, an amazing package which allows for very creative data visualisation in a fairly userfriendly manner. A good place to start learning about ggplot2 is http://www.cookbook-r.com/Graphs/

The following it an example of how to use `geom_trail` in the context of the inbuilt data set `amd`.

First, the data needs to be prepared. I often find that especially rather unexperienced users hesitate to manipulate / shape their data before doing any visualisation or analysis. 
...
(**Those simple precautions are**: Do not use functions in your script or report which write a file on your disk. This may easily overwrite your raw data! E.g. `write.csv` and friends should be used with great care)
...
On this note, I recommend to always start in a new session. This is another advantage of using rmarkdown. Knitting your script to a report makes you aware of inconsistencies in your script. 
...
Now we can use this and make a nice graph:
...
This one is with the lines and dots:
```
p + geom_trail(aes(group = age_cut10))
```

But you can also leave the points blank and include text instead:
```
...
- Thanks to **Alasdair Warwick** for both invaluable suggestions and testing, **Tim Yap** and **Siegfried Wagner** for great suggestions and **Abraham Olvera** for testing.
