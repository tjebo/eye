...
- [Tidy eye data](#myop): One column for "eye" (r/l) and dedicated columns for each eye-related variable

Further, it provides tools for easy data summaries and calculations:

- [Summarising data](#reveal) with common statistics (mean, sd, n, range)
- Summarise your data in a [blink of an eye](#blink)
...
There are erroneous visual acuity entries in this data set which I noticed during the work on this package. The data set curator has been contacted and it was concluded that these were erroneous entries
in the original medical health records. I decided to keep the values in the data set and wait for the final decision how to proceed from the data set curator (if they are going to replace it with missing 
values or not). I believe this is a great example for the challenges of real life data and a reminder to remain 
...
### Cleaning
- `NA` are assigned to missing entries or strings representing such entries 
...
As you can imagine, a lot of those steps rely hugely on reasonnable naming of your columns and this is what makes this function unfortunately a bit fragile. However, if you adhere to the naming conventions, blink (and myop) will do a great job for you.
