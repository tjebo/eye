...
Finally, *eye* includes [`geom_trail()`](#geom_trail) for some nice trail graphs. 
...
- Chart and VA conversion formulas are based on [@holladay], [@beck] and [@gregori]
...
messy_df <- data.frame(
  id = letters[1:3],
  iop_r_preop = sample(21:23),
  iop_r_postop = sample(11:13),
  iop_l_postop = sample(11:13),
  iop_l_preop = sample(31:33),
  va_r_preop = sample(41:43),
  va_l_preop = sample(41:43),
  va_r_postop = sample(51:53),
  va_l_postop = sample(45:47)
)
```
```{r myop two vars}
messy_df
...
### blink 
See your data in a blink of an eye - wrapper around [`myop`](#myop), [`eyes`](#eyes), [`va`](#va) and [`reveal`](#reveal)
```{r}
blink(messy_df)
```
...
**I do not assume responsability for your data or analysis**. Please remain
always wary when working with data. If you get results that do not make sense,
then it is likely that the data is in an unfortunate shape for which `eye` is
not suitable. I tried to think of many possible ways to deal with messy data,
but I am sure there will be far more creative ways out there to abuse data
entries. 

**eye will work smoother with clean data** (any package will, really!). 
[See here to learn about clean data.](https://tidyr.tidyverse.org/articles/tidy-data.html)
...
* Separate eye and VA codes with underscores ("bcva_l_preop", "VA_r", "left_va")
* Keep names short
