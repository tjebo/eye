...
### myop
Make your data long (myope). Convenience wrapper around `tidyr::pivot_longer`
```{r myop}
set.seed(42)
...
```
Often enough, data is a bit messy and there are right eye / left eye columns for more than one variable, e.g., for both IOP and VA. The following example shows one way to clean up this mess. 
```{r myop two vars}
library(dplyr)
